...
#define ROUND_UP(v, powerOf2Alignment) (((v) + (powerOf2Alignment)-1) & ~((powerOf2Alignment)-1))
	...
	void GenerateMengerSponge(std::vector<Vertex>& outputVertices,
	                          std::vector<UINT>& outputIndices)
	{
		struct Cube
		{
				...
				const XMVECTOR& bottomLeft4, const XMVECTOR& dx, const XMVECTOR& dy, bool flip)
			{
				...
				vertices.push_back(
					{ {bottomLeft.x, bottomLeft.y, bottomLeft.z, 1.f}, n, {1.f, 1.f, 0.f, 1.f}});
		...
		outputVertices.reserve(24 * cubes1.size());
		outputIndices.reserve(24 * cubes1.size());
